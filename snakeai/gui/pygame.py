...
class PyGameGUI:
    ...
    def run(self, num_episodes=1, attention=1, visualize=None):
        ...
        try:
            for episode in range(num_episodes):
                self.run_episode(attention, visualize=visualize)
                pygame.time.wait(1500)
        except QuitRequestedError:
            pass

    def run_episode(self, attention=1, visualize=None):
        """ Run the GUI player for a single episode. """

        # Initialize the environment.
        self.timestep_watch.reset()
        timestep_result, position = self.env.new_episode()
        ...
        while running:
            ...
            if timestep_timed_out or human_made_move:
                ...
                if not is_human_agent:
                    if visualize != None:
                        vis, action = self.agent.visualize(timestep_result.observation, position, timestep_result.reward, attention, visualize)
                    else:
                        action = self.agent.act(timestep_result.observation, position, timestep_result.reward, attention)
                
                if count == 0 and self.env.stats.fruits_eaten >= 10 and visualize != None:
                ...
                timestep_result, position = self.env.timestep()
