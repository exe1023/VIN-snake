...
class ExperienceReplay(object):
    ...
    def __init__(self, input_shape, num_actions, memory_size=100, attention=1):
        """
    ...
    def remember(self, state, position, action, reward, state_next, position_next, is_episode_end):
        """
    ...
    def get_batch(self, model, batch_size, discount_factor=0.9):
        ...
        for t in tmp:
            ...
            s.append((t[1][0], t[1][1]))
            s_next.append((t[2][0], t[2][1]))
        experience, s, s_next = np.array(experience), np.array(s), np.array(s_next)
        #print(experience, S)
        ...
        S = np.concatenate([s, s_next], axis=0)
        if self.attention == 0:
            y = model.predict([X, X, S] )
