...
class Environment(object):
    ...
    def record_timestep_stats(self, result):
        ...
        if self.verbose >= 1 and self.stats_file is None:
            ...
            self.stats_file = open('snake-env-' + str(timestamp) + '.csv', 'w')
        ...
        if self.verbose >= 2 and self.debug_file is None:
            ...
            self.debug_file = open('snake-env-' + str(timestamp) + '.log', 'w')
...
class TimestepResult(object):
    ...
    def __str__(self):
        ...
        return str(field_map) + '\nR = ' + str(self.reward) + '   end=' + str(self.is_episode_end) + '\n'
...
class EpisodeStatistics(object):
    ...
    def flatten(self):
        ...
        tmp = {}
        for action in ALL_SNAKE_ACTIONS:
            tmp['action_counter_' + str(action)] = self.action_counter.get(action, 0)
        #flat_stats.update({
        #    f'action_counter_{action}': self.action_counter.get(action, 0)
        #    for action in ALL_SNAKE_ACTIONS
        #})
        flat_stats.update(tmp)
        return flat_stats
